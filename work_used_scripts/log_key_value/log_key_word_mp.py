...
def procFile(word_count, fileOffset, keyWords, start_time, end_time, file):
    baseFile = os.path.basename(file)
    with open(file, "r") as READ_FILE:
        if fileOffset.has_key(baseFile):
            READ_FILE.seek(fileOffset[baseFile])
        lines     = []
        timeFlag  = False
        line      = READ_FILE.readline()
        offset = 0
        ...
        if not line: 
            if file_time_pat.search(baseFile): #已切割日志文件的最后，删除offset
                if fileOffset.has_key(baseFile):
                    del fileOffset[baseFile]
        else:
            fileOffset[baseFile] = offSet
...
def procFlow(data_file, lock, keyWordSet, fileOffset, q, exit_flag):
    while not exit_flag.is_set():
        ...
        for  root, dirs, files in os.walk(flowDir):
            for file in files: 
                ...
                lock.release()
                procFile(word_count, fileOffset, keyWords, start_time, end_time, file)
...
def procLogDir(data_file, keyWordSet, flows, isComplete):
    ...
    global FILE_OFFSET
    fileOffset = manager.dict(FILE_OFFSET)
    RLock = multiprocessing.RLock()
    processes=[]
    for i in range(NUM_OF_PROCESSES):
        processes.append(multiprocessing.Process(target=procFlow, \
            args=(data_file, RLock, keyWordSet, fileOffset, q, exit_flag)))
