...
expect $HOME/share/mywork/work_used_scripts/login.exp $PassWd1  10.180.214.$IP billapp
fi
...
expect $HOME/share/mywork/work_used_scripts/login.exp $PassWd2  10.180.214.$IP billmdb
fi

if [ $num -ge 10 -a $num -le 15 ]; then
echo "ssh billmdb@10.180.214.$IP"
echo "PassWord:$PassWd1"
expect $HOME/share/mywork/work_used_scripts/login.exp $PassWd1  10.180.214.$IP billmdb
fi
...
expect $HOME/share/mywork/work_used_scripts/login.exp Ailk@4$IP  10.180.214.$IP billroam
fi
...
expect $HOME/share/mywork/work_used_scripts/login.exp Ailk@14$IP  10.180.214.$IP billsrm
fi
...
echo "PassWord:$user"
expect $HOME/share/mywork/work_used_scripts/login.exp $user  10.10.10.183 $user
fi
